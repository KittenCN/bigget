...
def check_price(accountApi,markApi,orderApi,positionApi,symbol,marginCoin):
    ...
    try:
        ...
        if last_time != int(df.iloc[-1]['time']):
            ...
            if total_score != 0:
                content = "Date:{}, Product:{}, Price:{:.2f}, Score:{:.2f}, OpenSignal:{}, CloseSignal:{}, Signal_Generator:{}".format(current_datetime, symbol, current_price, total_score, current_open_signal, current_close_signal, signal_generator).ljust(len(content))
            ...
            if crossMaxAvailable >= total_amount * 0.3 and current_open_signal == "open_long":
                ...
                content = "Date:{}, Buy:{}, Side:{}, Price:{}, size:{}, presetStopLossPrice:{}, presetTakeProfitPrice:{}, status:{}".format(current_datetime, symbol, 'open_long', current_price, basecoin_size, round(current_price*StopLoss_rate, 1), round(current_price*TakeProfit_rate,1), order_result['msg']).ljust(len(content))
                print('\r' + content)
                write_txt(f"./log/log_{current_date}.txt", content + '\n')
            # open buy fail
            elif current_open_signal == "open_long":
                print('\rOpen_Long fail, crossMaxAvailable:{}, total_amount:{}'.format(crossMaxAvailable, total_amount)).ljust(len(content))
            basecoin_size = 0
            ...
            if (record_signal == "close_long" or current_open_signal == "close_long") and float(account_info['data']['unrealizedPL']) >= 0 and basecoin_size > 0:
                ...
                content = "Date:{}, Sell:{}, Side:{}, Price:{}, size:{}, status:{}".format(current_datetime, symbol, 'close_long', current_price, basecoin_size, order_result['msg']).ljust(len(content))
                print('\r' + content)
                write_txt(f"./log/log_{current_date}.txt", content + '\n')
            # close long fail
                ...
                print('\rClose_Long fail, unrealizedPL:{}'.format(float(account_info['data']['unrealizedPL']))).ljust(len(content))
                write_txt("./signal.txt", record_signal, rewrite=True)
            ...
            if crossMaxAvailable >= total_amount * 0.3 and current_open_signal == "open_short":
                ...
                content = "Date:{}, Sell:{}, Side:{}, Price:{}, size:{}, presetStopLossPrice:{}, presetTakeProfitPrice:{}, status:{}".format(current_datetime, symbol, 'open_short', current_price, basecoin_size, round(current_price*StopLoss_rate,1), round(current_price*TakeProfit_rate,1), order_result['msg']).ljust(len(content))
                print('\r' + content)
                write_txt(f"./log/log_{current_date}.txt", content + '\n')
            # open short fail
            elif current_open_signal == "open_short":
                print('\rOpen_Short fail, crossMaxAvailable:{}, total_amount:{}'.format(crossMaxAvailable, total_amount)).ljust(len(content))
            position_result = positionApi.single_position(symbol=symbol, marginCoin=marginCoin, print_info=False)
            basecoin_size = 0
            for position_element in position_result['data']:
            ...
            if (record_signal == "close_short" or current_open_signal == "close_short") and float(account_info['data']['unrealizedPL']) > 0 and basecoin_size > 0:
                ...
                content = "Date:{}, Sell:{}, Side:{}, Price:{}, size:{}, status:{}".format(current_datetime, symbol, 'close_short', current_price, basecoin_size, order_result['msg']).ljust(len(content))
                print('\r' + content)
                write_txt(f"./log/log_{current_date}.txt", content + '\n')
                ...
                print('\rClose_Short fail, unrealizedPL:{}'.format(float(account_info['data']['unrealizedPL']))).ljust(len(content))
                write_txt("./signal.txt", record_signal, rewrite=True)
